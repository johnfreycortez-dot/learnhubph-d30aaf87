...
function CourseDetail() { const { nicheId } = Route.useParams(); const navigate = useNavigate(); const [data,setData]=useState<any>(null);
  ...
  const isUnlocked=(lessonId:string)=>{const idx=orderedIds.indexOf(lessonId);return idx<=0||orderedIds.slice(0,idx).every((id:string)=>data.progress?.[id]);};
  return <StudentShell title={niche.NicheTitle}><div className="mx-auto max-w-4xl"><button onClick={()=>navigate({to:"/courses"})} className="inline-flex items-center gap-2 text-sm font-bold text-purple-700"><ArrowLeft size={16}/>All courses</button><section className="mt-5 rounded-2xl bg-white p-6 shadow-sm"><p className="text-sm font-bold text-purple-600">YOUR LEARNING PATH</p><h2 className="mt-2 text-3xl font-black">{niche.NicheTitle}</h2><p className="mt-2 text-sm text-gray-500">{done} of {lessons.length} lessons completed</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-purple-100"><div className="h-full rounded-full bg-purple-700" style={{width:`${percent}%`}}/></div></section><div className="mt-6 space-y-4">{niche.courses.flatMap((course:any)=>course.modules).map((module:any,index:number)=><section key={module.ModuleID} className="rounded-2xl bg-white p-5 shadow-sm"><h3 className="font-black">Module {index+1}: {module.Title}</h3><div className="mt-3 divide-y">{module.lessons.map((lesson:any)=>{const thumb=thumbUrl(data.thumbnailMap?.[lesson.LessonID]);const unlocked=isUnlocked(lesson.LessonID);const doneLesson=!!data.progress?.[lesson.LessonID];return <button key={lesson.LessonID} disabled={!unlocked} title={unlocked?undefined:"Complete the previous lesson to unlock"} onClick={()=>{if(!unlocked)return;navigate({to:"/lesson/$lessonId",params:{lessonId:lesson.LessonID},state:{modules:data.modules,niche:niche.NicheTitle} as any});}} className={`flex w-full items-center gap-3 py-3 text-left ${unlocked?"hover:text-purple-700":"cursor-not-allowed opacity-50"}`}>{doneLesson?<CheckCircle2 size={19} className="shrink-0 text-emerald-500"/>:unlocked?<Circle size={19} className="shrink-0 text-gray-300"/>:<Lock size={17} className="shrink-0 text-gray-300"/>}<span className="grid h-11 w-16 shrink-0 place-items-center overflow-hidden rounded-lg bg-purple-50">{thumb?<img src={thumb} alt="" loading="lazy" className="h-full w-full object-cover"/>:<PlayCircle size={18} className="text-purple-300"/>}</span><span className="flex-1 text-sm font-semibold">{lesson.Title}</span>{unlocked?<PlayCircle size={18} className="shrink-0"/>:<Lock size={16} className="shrink-0 text-gray-300"/>}</button>})}</div></section>)}</div></div></StudentShell>;
}
