...
// Position: fixed bottom-right so it sits above every page without
// interfering with sticky headers or sidebars.
export function ThemeToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={
        "fixed bottom-5 right-5 z-[100] inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white/90 px-3.5 py-2.5 text-xs font-bold text-gray-700 shadow-lg backdrop-blur-xl transition hover:scale-105 hover:text-purple-700 dark:border-white/10 dark:bg-gray-900/80 dark:text-gray-100 dark:hover:text-purple-300 " +
        className
      }
    >
